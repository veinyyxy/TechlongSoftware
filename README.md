...
- B5-J4b 已完成离线实现，但尚未部署到 AWS。它新增固定的 IAM-only 管理 Stack `techlong-s3-b5-cell-bootstrap-management`，集中持有 Manager、CloudFormation execution、Janitor 与 Scheduler 四组 boundary/role，共 8 个 IAM 资源，并把子 Bootstrap 的 author、execute 和 rollback 权限拆成短时、摘要绑定的 grant；正常流程固定为 `Locked → AuthorGrant → Locked → ExecuteGrant → Locked`，每个授权窗口后立即撤销，删除子 Bootstrap 才使用 `RollbackGrant → Locked`。child 模板按 raw SHA-256 内容寻址存入私有 build-source Bucket 的 `b5-cell-bootstrap/templates/sha256/<raw>.json`，明确位于 Provisioner 可写的 `source/*` 之外；AuthorGrant 只允许读取该 exact object，并把 `TemplateUrl`、CloudFormation `RoleARN`、deterministic `ChangeSetName` 和 4 种 `ResourceTypes` 全部锁定，ExecuteGrant 打开前还要对 exact child Change Set 与原始模板做预检。子 Stack `techlong-s3-b5-cell-bootstrap` 精确只有 4 个非 IAM cleanup/read-only 资源：1 天 STANDARD LogGroup、只读 inventory Lambda、Scheduler Group 和保持 `DISABLED` 的 15 分钟 Schedule；child 不拥有 IAM lifecycle，外置 execution role 仅可 `PassRole` 两个由管理栈持有的外部最小角色。没有 Cell Operator、付费 Cell execution role、Cell 删除权限或任何 VPC/ALB/ECS/RDS 资源。显式 `ProbeJanitor` 只允许两次空 inventory probe。模板对象很小，但 S3 存储与请求不能保证绝对零费用；离线完成不构成 AWS 部署或付费 Cell 批准，`registrationReady=false`、`liveReadbackReady=false`、`applyRuntimeReady=false`、`cleanupRuntimeReady=false` 全部保持不变。
...
- Shared Cell B3/B4 仍只生成 `renderOnly=true`、`applyReady=false` 的独立模板。模板现有独立 one-shot SG：零入站，出站仅 TCP 443 到公网和 5432 到 exact DB SG；DB 的 5432 入站只接受 app/one-shot 两个 SG，普通 app SG 仍只收 ALB:3000。只读 preflight 会验证公共 task subnet、SG/VPC/输出/所有权/TTL 绑定及这些 exact 规则。Sandbox 公网 IP 是无 NAT/endpoint 的低成本出站权衡，不等于开放入站；production 保持私网 `AssignPublicIp=DISABLED`。B5-J4b 已在仓库中把持有 8 个 IAM 资源的管理根与只含 4 个非 IAM 资源的 cleanup-only 子 Bootstrap 分离，并实现私有 S3 raw-digest TemplateUrl、exact Change Set/RoleARN/ResourceTypes 绑定、短窗 grant/立即撤销、严格 readback/IAM simulation 和空 inventory probe，但两者均尚未部署 AWS；子执行角色没有 IAM lifecycle，只能 Pass 两个管理栈外部最小角色，Schedule 固定 `DISABLED`，也没有 Cell apply/delete role、VPC、ALB、ECS 或 Aurora。未来 Cell TTL 仍固定 3 小时、租户 TTL 为 2 小时，创建/校正前还要求至少 15 分钟清理缓冲。

## 当前没有实现

- Paddle、自动续扣、Stripe 订阅模式、退款自动化、优惠券和复杂发票系统。
- 复杂发票、优惠券和自动退款。
- B5 的离线 ownership epoch、原子 authority 契约、可恢复 cleanup 状态机、AWS SDK v3 ECS/S3/Secrets Manager/DynamoDB 适配器源码、可信 receipt publisher/reader、订单服务 lifecycle 入口、真实 inspect-only Secrets Manager/PostgreSQL/S3 receipt provider、default-disabled Worker root composition、低成本 receipt/DynamoDB/IAM 支撑资源，以及严格校验和一次性 PostgreSQL 测试基础已经存在，但仍没有生产 material generator、完整的 PostgreSQL 变更/销毁 provider、approved baseline、Shared Cell 证据客户端 live root wiring、稳定且可清理的 Owner Secret 数据源，以及 Cell Bootstrap/Cell 的实际部署。订单服务只有 `POST /api/saas/provision` 已实现单调 epoch CAS，其他控制写接口尚未加 fence；新增 SQL/源码没有应用或部署。`0005`–`0007` 仍未应用到 Neon，`applyRuntimeReady=false`、`cleanupRuntimeReady=false`，因此当前不会真实创建 Cell、租户资源或自动回写正式入口。
...
下一步不是打开变量或运行现有 TaskDefinition。B5-J3 的 `tenant-lifecycle:1` 与 B5-J4a 的独立日志支撑已经注册/创建并严格回读；B5-J4b 的 8-IAM-resource 独立管理根、私有 S3 raw-digest TemplateUrl、临时 grant 状态机、4-non-IAM-resource cleanup-only 子 Bootstrap、严格 readback/IAM simulation 和空 inventory probe 也已完成离线实现，但尚未部署 AWS。下一个受控在线切片只能先部署并验证 J4b 管理 Stack，再严格按 `Locked → AuthorGrant → Locked → ExecuteGrant → Locked` 部署 cleanup-only 子 Stack 并运行空 probe；AuthorGrant 必须精确锁定 `TemplateUrl`、`RoleARN`、`ChangeSetName`、4 种 `ResourceTypes` 与 exact `GetObject`，ExecuteGrant 前必须完成 exact child Change Set/原始模板预检，并在每个窗口后立即 revoke。若回退子 Stack，只能走 `RollbackGrant → Locked`。该切片仍不得创建收费 Cell。账号仍没有 Shared Cell、公共 subnet、one-shot SG、RDS-managed Secret 或正向 management evidence，因此不能产出 runtime config，也不得执行 `RunTask`。随后才可单独复核真实 Cell/network/RDS Secret evidence、成本和 3 小时 TTL/cleanup，并另行决定是否批准一个付费 Sandbox Cell。之后再审查 `0005`、`0006`、`0007`、生产 material generator、完整 PostgreSQL 变更/销毁 provider、Worker live wiring、其余控制写接口 fence 和空租户 PostgreSQL 16.14 baseline。一次性 PostgreSQL 与受控 AWS Sandbox 中的迁移、并发 CAS、崩溃恢复和删除演练，以及 DNS/ACM/ACTIVE Trust Store，仍需分别批准。B5-I scoped rollback 脚本已经受审，但删除支撑数据的真实回退演练仍须在无租户状态下单独批准。所有门禁、真实 TTL 删除和费用核对通过后，才可另行批准一个付费 Sandbox Cell；`registrationReady`、`liveReadbackReady`、`applyRuntimeReady` 与 `cleanupRuntimeReady` 在此之前必须保持 `false`。升级/降级、退款和多实例仍不属于当前版本。模板使用说明见 [应用实例模板管理](./docs/app-instance-template-management.md)，执行门禁见 [AWS Sandbox S3 部署执行器](./docs/aws-sandbox-s3-worker.md)，本次边界见 [AWS Sandbox B5 实施边界](./docs/aws-sandbox-b5-implementation.md)。
