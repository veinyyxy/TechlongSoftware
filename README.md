...
下一步不是打开变量或运行现有 TaskDefinition。B5-J3 的 `tenant-lifecycle:1` 与 B5-J4a 的独立日志支撑已经注册/创建并严格回读，但账号仍没有 Shared Cell、公共 subnet、one-shot SG、RDS-managed Secret 或正向 management evidence，因此不能产出 runtime config，也不得执行 `RunTask`。下一个受控切片应先硬化仍 hard-disabled 的 Shared Cell Bootstrap：建立专用管理/回滚路径、默认移除长期 Cell Create/Execute/PassRole 能力、完成 exact Change Set/digest/IAM simulation/空扫描门禁；这一切仍不得创建收费 Cell。随后才单独复核真实 Cell/network/RDS Secret evidence、成本和 3 小时 TTL/cleanup，并另行批准一个付费 Sandbox Cell。之后再审查 `0005`、`0006`、`0007`、生产 material generator、完整 PostgreSQL 变更/销毁 provider、Worker live wiring、其余控制写接口 fence 和空租户 PostgreSQL 16.14 baseline。一次性 PostgreSQL 与受控 AWS Sandbox 中的迁移、并发 CAS、崩溃恢复和删除演练，以及 DNS/ACM/ACTIVE Trust Store，仍需分别批准。B5-I scoped rollback 脚本已经受审，但删除支撑数据的真实回退演练仍须在无租户状态下单独批准。所有门禁、真实 TTL 删除和费用核对通过后，才可另行批准一个付费 Sandbox Cell；`registrationReady`、`liveReadbackReady`、`applyRuntimeReady` 与 `cleanupRuntimeReady` 在此之前必须保持 `false`。升级/降级、退款和多实例仍不属于当前版本。模板使用说明见 [应用实例模板管理](./docs/app-instance-template-management.md)，执行门禁见 [AWS Sandbox S3 部署执行器](./docs/aws-sandbox-s3-worker.md)，本次边界见 [AWS Sandbox B5 实施边界](./docs/aws-sandbox-b5-implementation.md)。
