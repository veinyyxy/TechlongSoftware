...
J5g-f 完成本地契约后，J5g-g已准备好独立受审的J4c v2 plan-only consumer正向/反向上线门禁，但线上执行尚未开始。完成source/Manager身份刷新后，必须先在`LOCKED`下只读证明exact J4c-v1前态、`DISABLED` Schedule、Cell与authority均缺失，再分别取得临时IAM grant、Change Set创建与执行批准；不能把离线验证当作已部署。其后仍须独立评审 J5g-a 的专用 Cell operator/execution role线上边界，并完成 schema v2 production collector/root接线、真正可执行且 authority-bound 的失败回退与 TTL cleanup，以及 ACM、Trust Store、AZ/引擎/配额、空 tenant inventory和最坏费用的只读预检。当前 Shared Cell 模板仍是 `renderOnly=true`、`applyReady=false`；它虽然会创建 `ENABLED` 的三小时 TTL Schedule，但已部署 J4c Lambda 仍固定 `PLAN_ONLY` 且不接受 delete event，所以不能把该 Schedule 当成有效删除保证。在这些条件闭合并分别获得 IAM、Change Set 创建和付费 Execute 明确批准前，不得创建 Shared Cell、VPC、ALB、ECS service、RDS/Aurora 或 Route53。
