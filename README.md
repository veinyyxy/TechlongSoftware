...
下一步不是打开变量、重复部署 B5-J4b 或运行现有 TaskDefinition。B5-J3 的 inspect-default `tenant-lifecycle:2`（Build #7）、B5-J4a 的独立日志支撑，以及 B5-J4b 的 8-IAM-resource 独立管理根和 4-non-IAM-resource cleanup-only 子 Bootstrap 都已创建并严格回读；revision 1 已为 `INACTIVE`，注册临时权限已撤销，Bootstrap boundary 当前为 `v7/LOCKED`。J4b 已完成 `Locked → AuthorGrant → Locked → ExecuteGrant → Locked` 的受控上线、立即 revoke 和两次空 inventory probe，管理 Stack 当前必须继续保持 `LOCKED`，Schedule 必须继续保持 `DISABLED`。账号仍没有 Shared Cell、公共 subnet、one-shot SG、RDS-managed Secret 或正向 management evidence，因此当前不能产出 runtime config，也不得执行 `RunTask`。下一个受控切片应先离线实现并复核可信 provision predecessor、未来 live Stack evidence provenance 接口及独立安装边界；该契约闭合后，才单独复核真实 Cell/network/RDS Secret evidence、预计费用、3 小时 Cell TTL 和 cleanup 边界，并决定是否明确批准一个低成本但非零费用的 Sandbox Cell。在批准前不得创建 Shared Cell、VPC、ALB、ECS service、RDS/Aurora 或 Route53。之后再审查 `0005`、`0006`、`0007`、生产 material generator、尚未启用的 PostgreSQL 变更 provider、Worker live wiring、其余控制写接口 fence 和空租户 PostgreSQL 16.14 baseline。一次性 PostgreSQL 与受控 AWS Sandbox 中的迁移、并发 CAS、崩溃恢复和删除演练，以及 DNS/ACM/ACTIVE Trust Store，仍需分别批准。B5-I scoped rollback 脚本已经受审，但删除支撑数据的真实回退演练仍须在无租户状态下单独批准。所有门禁、真实 TTL 删除和费用核对通过后，才可另行批准一个付费 Sandbox Cell；`registrationReady`、`liveReadbackReady`、`applyRuntimeReady` 与 `cleanupRuntimeReady` 在此之前必须保持 `false`。升级/降级、退款和多实例仍不属于当前版本。模板使用说明见 [应用实例模板管理](./docs/app-instance-template-management.md)，执行门禁见 [AWS Sandbox S3 部署执行器](./docs/aws-sandbox-s3-worker.md)，本次边界见 [AWS Sandbox B5 实施边界](./docs/aws-sandbox-b5-implementation.md)。
