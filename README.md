...
- B5-J4a 已完成独立 lifecycle LogGroup 支撑的离线 IaC/操作/readback 契约，但尚未部署到 AWS。Stack 固定为 `techlong-sandbox-tenant-b5j4logs`，只允许一个 `STANDARD` 日志组 `/saas/cell-sandbox-1/tenant-lifecycle`，保留 1 天，无 KMS、stream、filter、ECS 或 Cell。Provisioner 只能通过 exact CloudFormation execution role 创建，Logs 直接只读回读使用现有 source AWS CLI `login_session` profile，不扩展 IAM。`ExpiresAt` 不会触发 Janitor 自动删除；B5-J3 模板不改、Cell Bootstrap 仍 hard-disabled、四个 gate 仍为 `false`，任何路径都不得执行 `RunTask`。
...
下一步不是打开变量或运行现有 TaskDefinition。B5-J3 的 `tenant-lifecycle:1` 已注册并回读，而 B5-J4a 的 lifecycle 日志支撑目前只是离线实现，不能把模板存在描述为 AWS 资源已创建。下一个受控在线动作应在刷新 source `login_session` 和 Provisioner MFA AssumeRole 后，只创建/回读单资源 Stack `techlong-sandbox-tenant-b5j4logs`；执行前后都必须确认 `cell-sandbox-1=MISSING`，并对 CloudFormation original template、唯一资源、exact Logs 属性/标签以及零 stream/零 stored bytes 做 fail-closed 回读；不扩展 IAM，不执行 `RunTask`。完成该在线证据后，再单独审查仍 hard-disabled 的 Shared Cell Bootstrap、真实 Cell/network/RDS Secret evidence、成本和 TTL/cleanup；账号在没有 Shared Cell、公共 subnet、one-shot SG、RDS-managed Secret 和正向 management evidence 时不能产出 runtime config。随后再审查 `0005`、`0006`、`0007`、生产 material generator、完整 PostgreSQL 变更/销毁 provider、Worker live wiring、其余控制写接口 fence 和空租户 PostgreSQL 16.14 baseline。一次性 PostgreSQL 与受控 AWS Sandbox 中的迁移、并发 CAS、崩溃恢复和删除演练，以及 DNS/ACM/ACTIVE Trust Store，仍需分别批准。B5-I scoped rollback 脚本已经受审，但删除支撑数据的真实回退演练仍须在无租户状态下单独批准。所有门禁、真实 TTL 删除和费用核对通过后，才可另行批准一个付费 Sandbox Cell；`registrationReady`、`liveReadbackReady`、`applyRuntimeReady` 与 `cleanupRuntimeReady` 在此之前必须保持 `false`。升级/降级、退款和多实例仍不属于当前版本。模板使用说明见 [应用实例模板管理](./docs/app-instance-template-management.md)，执行门禁见 [AWS Sandbox S3 部署执行器](./docs/aws-sandbox-s3-worker.md)，本次边界见 [AWS Sandbox B5 实施边界](./docs/aws-sandbox-b5-implementation.md)。
