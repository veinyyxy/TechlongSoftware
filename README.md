...
在线核验显示：AWS CLI Profile `techlong-sandbox-user` 固定在 Region `ca-central-1`、Account `402010193138`，身份为 `arn:aws:iam::402010193138:user/techlong-sandbox-dev`；该 IAM User 绑定 exact MFA，本机 `techlong-sandbox-provisioner` Profile 产生的会话身份为 `arn:aws:sts::402010193138:assumed-role/TechlongSandboxProvisionerRole/techlong-sandbox-provisioner`，核验过程没有读取或输出密钥。S3-A 与 2026-08-22 B5-I Bootstrap 已部署既有低成本支撑资源。2026-08-24 的 `LifecycleReadback` Change Set `techlong-s3-b5-support-lifecycle-readback-60d854ad2664718e`（raw SHA-256 `60d854ad2664718eed88ec4731ff3a70cb84b34ba9dcaf439782dcba7a816113`，canonical SHA-256 `1fe4af4b94a198437511a147fe05685eefb768304e3ab487ca06722657c2223b`）只对 `ServiceRoleBoundary`、`ProvisionerBoundary`、`TenantLifecycleTaskRole`、`DeploymentWorkerRole` 执行四项无 replacement 修改，`techlong-s3-bootstrap` 最终为 `UPDATE_COMPLETE`；线上回读的两个 managed-policy 默认版本分别为 `v3`、`v4`。随后 Build #4 从后端提交 `f4aa0febeba526f737bac3b59d516e1ab5c24482` 构建不可变镜像 `sha256:4815009949cd5219add56fedb183f1809b728081562f0280ede5229b567136f0`，全阶段成功且 ECR 扫描 `COMPLETE`、0 findings；该镜像尚未写入 execution binding。仍未创建 Cell、ALB、ECS、Aurora/RDS、VPC、DNS、TaskDefinition 或租户 Stack，也没有执行 `RunTask`。进入真实租户 Apply 前，应继续使用短期角色会话；不要在仓库或 `.env.local` 中配置长期 `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY`。
