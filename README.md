...
- B5-J5g-a-online-1 已把专用 Cell lifecycle IAM 的 `Locked` 管理根扩展为受控线上 controller。固定 management Stack 为 `techlong-s3-b5-cell-lifecycle-management`；入口提供 `LocalValidate`、`OnlineValidate`、`CreateChangeSet`、`InspectChangeSet`、`ExecuteChangeSet`、`Readback`，且 `UpdateShape` 只接受 `InitialLocked`。本切片只允许创建和严格回读四资源、无临时 grant 的 Locked IAM root；原有 `AuthorGrant`、`ExecuteGrant`、`RollbackGrant` 仍只能离线渲染，不能通过该 controller 部署，也不批准付费 Cell。`2026-09-13` 的只读 `OnlineValidate` 已以 raw/canonical SHA-256 `acaa2b5e65361b84ed8fd3fb55d5a512b64bbf51b9fd855837d0adeef2f06bc1` / `803f28a0741f411493852dbaa2282fba2aa109b4f64b4986ee85d97e39eccda5` 通过；随后已创建但未执行 InitialLocked Change Set `arn:aws:cloudformation:ca-central-1:402010193138:changeSet/techlong-s3-b5-cell-lifecycle-management-initial-locked-56020f776e9dc476/c546bbff-48b6-40b7-b245-a575d8307f11`，对应 StackId 为 `arn:aws:cloudformation:ca-central-1:402010193138:stack/techlong-s3-b5-cell-lifecycle-management/fb742b50-afb2-11f1-85b7-02588681429d`。两次独立 `InspectChangeSet` 均确认 Stack 为 `REVIEW_IN_PROGRESS`、Change Set 为 `CREATE_COMPLETE` / `AVAILABLE`，变更恰好是 Add 两个 `AWS::IAM::ManagedPolicy` 与两个 `AWS::IAM::Role`，四项 `Replacement=null`、`Scope/Details` 为空且 `RollbackConfiguration` canonical exact `{}`。首次创建后的后置核验曾因 PowerShell `@($null).Count` 对空对象的误判而 fail closed；门禁改为要求顶层字段存在且 canonical exact `{}` 后，两次独立 Inspect 通过。Change Set 从未执行，四个 IAM实体仍未创建，Cell 为 `MISSING`、authority key为 `ABSENT`；`ExecuteChangeSet` 仍需另一项独立明确确认，四个 readiness gate保持 `false`。AWS 对普通 Stack 的 CloudFormation service-role trust 是否稳定提供 `aws:SourceArn` / `aws:SourceAccount` 没有官方保证，因此当前继续使用 service principal trust，并依靠 exact boundary、inline policy、Change Set 与 readback收口。
...
J5g-g 已按独立受审的 `Locked → AuthorGrant → Locked → ExecuteGrant → Locked` 序列完成 J4c v2 plan-only consumer 上线，并通过严格 ZIP/readback 与两次 `ABSENT_SAFE` 探针；它仍固定 `PLAN_ONLY`、不接受 delete event，不能被解释为真实清理能力。J5g-a-online-1 现已实现专用 Cell operator/execution role 的 Locked IAM root controller，完成只读 `OnlineValidate`，并创建、两次独立检查了 InitialLocked Change Set；该 Change Set为 `CREATE_COMPLETE` / `AVAILABLE`，management Stack只是 `REVIEW_IN_PROGRESS` placeholder，尚未执行且四个 IAM实体尚未创建。下一动作若继续，必须单独明确批准 `ExecuteChangeSet`，且仍只能使用 `UpdateShape=InitialLocked`。同时还须完成 schema v2 production collector/root 接线、真正可执行且 authority-bound 的失败回退与 TTL cleanup，以及 ACM、Trust Store、AZ/引擎/配额、空 tenant inventory 和最坏费用的只读预检。当前 Shared Cell 模板仍是 `renderOnly=true`、`applyReady=false`；它虽然会创建 `ENABLED` 的三小时 TTL Schedule，但当前 Janitor 不接受该删除事件，所以不能把该 Schedule 当成有效删除保证。在这些条件闭合并分别获得 IAM执行和付费 Cell Execute明确批准前，不得创建 Shared Cell、VPC、ALB、ECS service、RDS/Aurora 或 Route53。
