...
- B5-J2 的订单服务源码已实现真实 inspect-only production composition：只用 RDS-managed Secret ARN 读取 exact username/password，以校验过的 management endpoint/port/database 和目标 database/role 建立 PostgreSQL TLS 只读连接，读取有界 catalog/ownership marker，再通过既有 immutable publisher 写入受约束的 S3 raw receipt。该入口不实现 `prepare_empty_database`、`restore_approved_baseline`、`migrate_saas`、`verify` 或 `destroy`，也没有运行 ECS task。对应 CloudFormation IAM 变更已通过 `LifecycleReadback` 三阶段增量 Change Set 部署并在线回读：只修改两个 boundary 和两个 role，全部无 replacement；Build #4 也已由当前后端提交构建并完成零发现扫描。该阶段结束时还没有 TaskDefinition；runtime config 与四个 readiness gate 均未打开。
- B5-J3 已通过单资源 Stack `techlong-sandbox-tenant-b5j3` 注册并严格回读 `arn:aws:ecs:ca-central-1:402010193138:task-definition/tenant-lifecycle:1`。它固定 Build #4 digest、`inspect` 命令、`256/512` Fargate、nonroot/只读 root filesystem、零 Secret/端口/卷/sidecar；部署模板 canonical SHA-256 为 `f8d9993c47c0c332e79de6aeb845d4946464ebdf4e79719d3c5696ec380dc475`，回读证据 canonical SHA-256 为 `012c60f92b698f0c0c1e633b04a925b9544ff0cc91cbdc5004eca1e73617fbc8`。注册所需的 LifecycleTaskRole PassRole 只在受审窗口临时加入，随后由 `LifecycleTaskRegistrationRevoke` Change Set 移除；Bootstrap 当前 canonical SHA-256 为 `112d1b47807962b2ae3e3d751c2b6d2d9cb48c1660f2aa75d24991ebf9cbdf14`，IAM 默认版本为 `v3`。精确 `cell-sandbox-1` cluster 回读为 `MISSING`，日志组也不由本阶段创建，因此没有 `RunTask`、Cell 或 runtime config；四个 readiness gate 继续为 `false`。
...
- Shared Cell B3/B4 仍只生成 `renderOnly=true`、`applyReady=false` 的独立模板。模板现有独立 one-shot SG：零入站，出站仅 TCP 443 到公网和 5432 到 exact DB SG；DB 的 5432 入站只接受 app/one-shot 两个 SG，普通 app SG 仍只收 ALB:3000。只读 preflight 会验证公共 task subnet、SG/VPC/输出/所有权/TTL 绑定及这些 exact 规则。Sandbox 公网 IP 是无 NAT/endpoint 的低成本出站权衡，不等于开放入站；production 保持私网 `AssignPublicIp=DISABLED`。B5-J4b 已把持有 8 个 IAM 资源的管理根与只含 4 个非 IAM 资源的 cleanup-only 子 Bootstrap 分离，并在 AWS 完成部署、严格 readback/IAM simulation 与两次空 inventory probe；管理 Stack 最终为 `UPDATE_COMPLETE`/`LOCKED`，子 Stack 为 `CREATE_COMPLETE`。子执行角色没有 IAM lifecycle，只能 Pass 两个管理栈外部最小角色，Schedule 固定 `DISABLED`，Lambda 没有 reserved concurrency；账号仍没有 Shared Cell、Cell apply/delete role、VPC、ALB、ECS cluster/service 或运行中的 task、RDS、Route 53，也没有执行 `RunTask`，既有 inspect-only `tenant-lifecycle:1` TaskDefinition 不变。未来 Cell TTL 仍固定 3 小时、租户 TTL 为 2 小时，创建/校正前还要求至少 15 分钟清理缓冲。

## 当前没有实现

- Paddle、自动续扣、Stripe 订阅模式、退款自动化、优惠券和复杂发票系统。
- 复杂发票、优惠券和自动退款。
- B5 的离线 ownership epoch、原子 authority 契约、可恢复 cleanup 状态机、AWS SDK v3 ECS/S3/Secrets Manager/DynamoDB 适配器源码、可信 receipt publisher/reader、订单服务 lifecycle 入口、真实 inspect-only Secrets Manager/PostgreSQL/S3 receipt provider、default-disabled Worker root composition、低成本 receipt/DynamoDB/IAM 支撑资源、已上线且保持锁定的 cleanup-only Cell Bootstrap，以及严格校验和一次性 PostgreSQL 测试基础已经存在，但仍没有生产 material generator、完整的 PostgreSQL 变更/销毁 provider、approved baseline、Shared Cell 证据客户端 live root wiring、稳定且可清理的 Owner Secret 数据源，以及 Shared Cell 的实际部署。订单服务只有 `POST /api/saas/provision` 已实现单调 epoch CAS，其他控制写接口尚未加 fence；新增 SQL/源码没有应用或部署。`0005`–`0007` 仍未应用到 Neon，`registrationReady=false`、`liveReadbackReady=false`、`applyRuntimeReady=false`、`cleanupRuntimeReady=false`，因此当前不会真实创建 Cell、租户资源或自动回写正式入口。
...
在线核验显示：AWS CLI Profile `techlong-sandbox-user` 固定在 Region `ca-central-1`、Account `402010193138`，身份为 `arn:aws:iam::402010193138:user/techlong-sandbox-dev`；该 IAM User 绑定 exact MFA，本机 `techlong-sandbox-provisioner` Profile 产生的会话身份为 `arn:aws:sts::402010193138:assumed-role/TechlongSandboxProvisionerRole/techlong-sandbox-provisioner`，核验过程没有读取或输出密钥。S3-A、2026-08-22 B5-I、2026-08-24 `LifecycleReadback` 与 Build #4 均已部署/构建并独立回读。B5-J3 进一步创建了只含一个 `AWS::ECS::TaskDefinition` 的 `techlong-sandbox-tenant-b5j3` Stack；输出精确绑定 `tenant-lifecycle:1`，镜像为 Build #4 digest，命令仅为 `inspect`。临时注册 grant 和随后 revoke 的 Change Set 都只修改 `ExecutionRoleBoundary` 且 `Replacement=False`；最终 policy `v3` 的 PassRole 仅保留 TaskExecutionRole 与普通 TaskRole。2026-08-26，B5-J4b 的 8-IAM-resource 管理 Stack 已达到 `UPDATE_COMPLETE` 并回到 `LOCKED`，4-resource cleanup-only 子 Stack 已达到 `CREATE_COMPLETE`；严格 readback 与两次空 inventory probe 均通过，Schedule 为 `DISABLED`，Lambda 未配置 reserved concurrency。精确 Cell cluster 仍为 `MISSING`，账号未创建 Shared Cell、ALB、Aurora/RDS、VPC、Route53、租户服务或正式租户 Stack，也没有执行 `RunTask`。进入真实租户 Apply 前，应继续使用短期角色会话；不要在仓库或 `.env.local` 中配置长期 `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY`。
...
下一步不是打开变量、重复部署 B5-J4b 或运行现有 TaskDefinition。B5-J3 的 `tenant-lifecycle:1`、B5-J4a 的独立日志支撑，以及 B5-J4b 的 8-IAM-resource 独立管理根和 4-non-IAM-resource cleanup-only 子 Bootstrap 都已创建并严格回读；J4b 已完成 `Locked → AuthorGrant → Locked → ExecuteGrant → Locked` 的受控上线、立即 revoke 和两次空 inventory probe，管理 Stack 当前必须继续保持 `LOCKED`，Schedule 必须继续保持 `DISABLED`。账号仍没有 Shared Cell、公共 subnet、one-shot SG、RDS-managed Secret 或正向 management evidence，因此当前不能产出 runtime config，也不得执行 `RunTask`。下一个受控切片应先单独复核未来真实 Cell/network/RDS Secret evidence、预计费用、3 小时 Cell TTL 和 cleanup 边界，再决定是否明确批准一个低成本但非零费用的 Sandbox Cell；在批准前不得创建 Shared Cell、VPC、ALB、ECS service、RDS/Aurora 或 Route53。之后再审查 `0005`、`0006`、`0007`、生产 material generator、完整 PostgreSQL 变更/销毁 provider、Worker live wiring、其余控制写接口 fence 和空租户 PostgreSQL 16.14 baseline。一次性 PostgreSQL 与受控 AWS Sandbox 中的迁移、并发 CAS、崩溃恢复和删除演练，以及 DNS/ACM/ACTIVE Trust Store，仍需分别批准。B5-I scoped rollback 脚本已经受审，但删除支撑数据的真实回退演练仍须在无租户状态下单独批准。所有门禁、真实 TTL 删除和费用核对通过后，才可另行批准一个付费 Sandbox Cell；`registrationReady`、`liveReadbackReady`、`applyRuntimeReady` 与 `cleanupRuntimeReady` 在此之前必须保持 `false`。升级/降级、退款和多实例仍不属于当前版本。模板使用说明见 [应用实例模板管理](./docs/app-instance-template-management.md)，执行门禁见 [AWS Sandbox S3 部署执行器](./docs/aws-sandbox-s3-worker.md)，本次边界见 [AWS Sandbox B5 实施边界](./docs/aws-sandbox-b5-implementation.md)。
