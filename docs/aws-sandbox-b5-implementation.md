...
3. ECS one-shot、exact-five-key Secret、trusted S3 receipt reader/publisher、订单服务 inspect 与 cleanup-only destroy 源码、单 revision TaskDefinition intent 和 runtime-provenance-checked management target compiler已存在；receipt Bucket/专用 LifecycleTaskRole 已部署。已注册并 readback 的 `tenant-lifecycle:1` 仍固定旧 Build #4 inspect-only artifact；当前 destroy 源码尚未 build/register/readback/RunTask。生产 material generator、其他数据库变更操作、approved baseline、Worker live root wiring 和真实崩溃演练仍未完成。
...
6. 分阶段 cleanup coordinator 与 exact provision predecessor 接线已离线实现，backend 本地源码已有 inspect 和 cleanup-only destroy provider；但 destroy 新 artifact、Cell Janitor/standalone Worker live destroy root wiring，以及真实 provider-side 删除演练仍未完成。
