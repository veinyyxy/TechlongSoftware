...
- 生命周期renderer新增offline-only `AuthorCompensationGrant`候选：metadata只记录approved plan SHA-256、exact Change Set ARN和独立短窗口，尚不从normalized candidate重算/验证plan；IAM写权限只包含exact StackId上的`DeleteChangeSet`与`DeleteStack`，按授权模型绑定exact Change Set name、四个resource tag、review下界与动作级expiry前10/5分钟截止，并以`cloudformation:RoleArn`请求键必须缺失来禁止role override。IAM不能证明占位栈状态、零资源或两步顺序，所以该组合grant明确标记需在上线前拆分为DeleteChangeSet/DeleteStack两个阶段，并由controller exact-match plan。纯核心也没有跨进程持久锁，controller必须为同一plan提供single-flight/持久claim。它没有Create/Execute/PassRole/S3，也没有给CloudFormation execution role挂载inline policy；`CloudApplyEnabled=false`、`CompensationPlanDigestVerified=false`、`CompensationSafeToAuthorRevoke=false`。PowerShell wrapper只暴露`LocalValidate`，runtime root没有导入该模块。本切片未调用AWS、未改线上IAM、未创建/删除任何Stack或付费资源；下一小阶段必须先实现production AWS adapter、受控management/child controller、plan重算、持久claim与split grants，再分别取得两段补偿及AuthorRevoke写批准。
